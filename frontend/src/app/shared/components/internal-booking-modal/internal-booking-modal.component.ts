...
@Component({
  ...
  template: `
    <div class="fixed inset-0 z-[100] bg-black/60 backdrop-blur-sm p-4 flex items-center justify-center">
      <div class="rounded-2xl shadow-2xl w-full max-w-[1300px] max-h-[98vh] flex flex-col text-neutral-800 animate-fadeIn relative bg-white overflow-hidden">
        
        <!-- Header -->
        <div class="px-8 py-5 flex items-center justify-between shrink-0 bg-white border-b border-neutral-100 relative">
          <div class="flex items-center gap-8 w-full">
            <h2 class="text-3xl font-bold font-serif text-black tracking-tight shrink-0">New Appointment</h2>
            
            <div class="h-10 border-l-2 border-dotted border-neutral-200 shrink-0"></div>
          ...
          <button type="button" (click)="close.emit()" class="text-neutral-500 hover:text-black transition-colors shrink-0 absolute right-8 top-1/2 -translate-y-1/2">
        ...
        <div class="bg-dot-pattern relative flex flex-col w-full h-full">
          ...
          <div class="flex-1 flex overflow-hidden px-8 pb-8 gap-8 items-stretch">
            ...
            <form *ngIf="!success && !showResumePrompt" [formGroup]="form" class="flex-1 flex flex-col justify-between overflow-y-auto custom-scrollbar pr-4 pb-4 min-h-0">
              
              <div class="flex gap-5 flex-1 items-start">
                
                <!-- Left Column (Smaller Width) -->
                <div class="flex flex-col gap-5 w-[380px] shrink-0">
                  <!-- 1. Client Details -->
                <div class="bg-white p-5 rounded-2xl border border-neutral-100 shadow-sm flex flex-col">
                  <h3 class="text-xl font-bold font-serif text-black mb-3 tracking-wide">1. Client Details</h3>
                  ...
                  <!-- 3. Choose Location -->
                <div class="bg-white p-5 rounded-2xl border border-neutral-100 shadow-sm flex flex-col">
                  <h3 class="text-xl font-bold font-serif text-black mb-3 tracking-wide">3. Choose Location</h3>
                  <div class="grid flex flex-col gap-4">
                    <!-- Location 1 -->
                    <div (click)="form.patchValue({locationId: 2})" [class.border-[#d8972e]]="form.value.locationId === 2" [class.shadow-sm]="form.value.locationId === 2" class="bg-white border border-neutral-200 rounded-xl p-4 cursor-pointer flex gap-3 items-start transition-all">
                    ...
                    <!-- Location 2 -->
                    <div (click)="form.patchValue({locationId: 1})" [class.border-[#d8972e]]="form.value.locationId === 1" [class.shadow-sm]="form.value.locationId === 1" class="bg-white border border-neutral-200 rounded-xl p-4 cursor-pointer flex gap-3 items-start transition-all">
                ...
                <!-- Right Column (Wider for Date/Time) -->
                <div class="flex flex-col gap-5 flex-1 min-w-0">
                  <!-- 2. Select Services -->
                <div class="bg-white p-5 rounded-2xl border border-neutral-100 shadow-sm flex flex-col">
                  <h3 class="text-xl font-bold font-serif text-black mb-3 tracking-wide">2. Select Services</h3>
                  ...
                  <div class="rounded-lg flex flex-col gap-2 max-h-[140px] overflow-y-auto custom-scrollbar pr-2">
                    <div *ngFor="let s of filteredServices" (click)="selectService(s)"
                         ...
                         class="bg-white px-4 py-3 border rounded-xl flex items-center justify-between cursor-pointer hover:border-[#d8972e] transition-all">
                      ...
                      <div class="text-sm font-bold text-neutral-900 flex-1 text-right mr-4">JMD $ <br class="hidden" />{{ s.price_jmd | number:'1.0-0' }}</div>
                  ...
                  <!-- 4. Date & Time -->
                <div class="bg-white p-5 rounded-2xl border border-neutral-100 shadow-sm flex flex-col">
                  <h3 class="text-xl font-bold font-serif text-black mb-3 tracking-wide">4. Date & Time</h3>
                  
                  <div class="flex flex-col lg:flex-row gap-5 flex-1 items-start">
                    
                    <div class="flex flex-col w-full lg:w-[260px] shrink-0">
                      <!-- Calendar -->
                      <div class="bg-[#0a0a0a] text-white rounded-xl p-3 shadow-lg flex flex-col">
                        ...
                        <div class="grid grid-cols-7 gap-y-2 gap-x-1 content-start">
                          <button type="button" *ngFor="let day of calendarDays" 
                                  ...
                                  [class.shadow-sm]="form.value.date === day.fullDate"
                      ...
                      <div class="flex items-center justify-center gap-4 mt-2">
                        <div class="flex items-center gap-1.5">
                          <mat-icon class="!text-[#d8972e] opacity-50 !w-5 !h-5 !text-[20px]">workspace_premium</mat-icon>
                          <span class="text-[9px] font-bold text-neutral-600 tracking-wider">BOOKED</span>
                        </div>
                        <div class="flex items-center gap-1.5">
                          <div class="w-4 h-4 rounded-full border border-[#d8972e]"></div>
                          <span class="text-[9px] font-bold text-neutral-600 tracking-wider">SELECTED</span>
                        </div>
                      </div>
                    </div>
                    ...
                    <div class="flex-1 w-full">
                      ...
                      <div class="grid grid-cols-2 xl:grid-cols-3 gap-2 max-h-[160px] overflow-y-auto custom-scrollbar pr-2">
                        <button type="button" *ngFor="let slot of timeSlots" (click)="selectTimeSlot(slot)"
                                ...
                                class="py-2.5 px-2 border rounded-lg text-xs font-bold transition-all text-center">
                          {{ slot }}
                        </button>
                      </div>
                    </div>
            ...
            <div class="w-[320px] shrink-0 bg-[#fbf7f0] p-7 flex flex-col justify-between rounded-2xl relative shadow-md z-10">
              ...
              <div>
                <h3 class="text-[#d8972e] font-serif text-base font-bold tracking-widest mb-8 uppercase">Appointment Summary</h3>
                ...
                <div class="space-y-6">
                  ...
                  <!-- Date & Time -->
                  <div class="pb-2">
                    <div class="text-[10px] text-[#d8972e] font-bold tracking-widest uppercase mb-2">Date & Time</div>
                    <div class="text-sm font-black text-neutral-900 mb-1">
                      {{ form.value.date | date:'longDate' }}
                    </div>
                    <div class="text-xs text-neutral-700 font-bold bg-neutral-100 py-1.5 px-3 rounded-md inline-block">
                      {{ formattedTime }}
                    </div>
                  </div>
