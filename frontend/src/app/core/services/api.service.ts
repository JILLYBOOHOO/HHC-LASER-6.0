...
import {
  ApiResponse, Service, ServiceCategory, Employee, EmployeeSchedule,
  Location, Appointment, CreateBookingDto, MembershipPlan, CustomerMembership,
  Transaction, GalleryImage, Testimonial, Promotion, TreatmentNote, IntakeForm, DashboardStats
} from '../models/models';
...
export class ApiService {
  ...
  getServices(categoryId?: number, isFeatured?: boolean): Observable<ApiResponse<Service[]>> {
  ...
  updateService(id: number, service: Partial<Service>): Observable<ApiResponse<{message: string}>> {
    return this.http.put<ApiResponse<{message: string}>>(`${this.base}/services/${id}`, service);
  }

  deleteService(id: number): Observable<ApiResponse<{message: string}>> {
    return this.http.delete<ApiResponse<{message: string}>>(`${this.base}/services/${id}`);
  }
  ...
  getEmployeeSchedule(employeeId: number): Observable<ApiResponse<EmployeeSchedule[]>> {
  ...
  getAvailableSlots(employeeId: number, locationId: number, date: string, duration: number): Observable<ApiResponse<string[]>> {
    ...
    return this.http.get<ApiResponse<string[]>>(`${this.base}/bookings/available-slots`, { params });
  }

  // ─── Bookings ───────────────────────────────────────────────────────────────
  createBooking(dto: CreateBookingDto): Observable<ApiResponse<{ appointment: Appointment; payment: any }>> {
    return this.http.post<ApiResponse<{ appointment: Appointment; payment: any }>>(`${this.base}/bookings`, dto);
  }
  ...
  updateBookingStatus(appointmentId: number, status: string, notes?: string): Observable<ApiResponse> {
  ...
  getPaymentHistory(): Observable<ApiResponse<{ transactions: Transaction[] }>> {
    return this.http.get<ApiResponse<{ transactions: Transaction[] }>>(`${this.base}/payments/history`);
  }

  // ─── Memberships ────────────────────────────────────────────────────────────
  getMembershipPlans(): Observable<ApiResponse<MembershipPlan[]>> {
    return this.http.get<ApiResponse<MembershipPlan[]>>(`${this.base}/memberships/plans`);
  }

  getMyMemberships(): Observable<ApiResponse<CustomerMembership[]>> {
    return this.http.get<ApiResponse<CustomerMembership[]>>(`${this.base}/memberships/my`);
  }

  subscribeMembership(planId: number, autoRenew = true): Observable<ApiResponse<{ membershipId: number }>> {
    return this.http.post<ApiResponse<{ membershipId: number }>>(`${this.base}/memberships/subscribe`, {
      plan_id: planId,
  ...
  cancelMembership(membershipId: number): Observable<ApiResponse> {
    return this.http.patch<ApiResponse>(`${this.base}/memberships/${membershipId}/cancel`, {});
  }

  // ─── Medical ─────────────────────────────────────────────────────────────────
  getIntakeForm(userId: number): Observable<ApiResponse<IntakeForm>> {
    return this.http.get<ApiResponse<IntakeForm>>(`${this.base}/medical/${userId}/intake`);
  }

  submitIntakeForm(userId: number, form: IntakeForm): Observable<ApiResponse<{ id: number }>> {
    return this.http.post<ApiResponse<{ id: number }>>(`${this.base}/medical/${userId}/intake`, form);
  }

  getTreatmentHistory(userId: number): Observable<ApiResponse<TreatmentNote[]>> {
    return this.http.get<ApiResponse<TreatmentNote[]>>(`${this.base}/medical/${userId}/treatment-history`);
  }
  ...
  getAdminCustomers(page = 1, limit = 20, search?: string): Observable<ApiResponse<any[]>> {
  ...
  getRevenueReport(from?: string, to?: string, locationId?: number): Observable<ApiResponse<any>> {
    ...
    if (locationId) params = params.set('location_id', locationId);
    return this.http.get<ApiResponse<any>>(`${this.base}/admin/reports/revenue`, { params });
  }
}
