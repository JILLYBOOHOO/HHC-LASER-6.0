...
import { executeQuery, withTransaction } from '../config/database';
...
router.get('/business', async (req: Request, res: Response): Promise<void> => {
  try {
    const rows = await executeQuery<{ setting_key: string; setting_value: unknown }>(
    ...
    await withTransaction(async (conn) => {
      for (const [key, value] of Object.entries(settings)) {
        await conn.execute(
          ...
          [key, JSON.stringify(value), 'Updated via settings page']
        );
      }
    });
